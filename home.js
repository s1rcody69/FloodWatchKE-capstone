...
function getAllReports() {
  ...
  const activeStored = stored.filter(report => - report.time < twentyFourHours);

  //Saving the cleaned list back to the localStorage
  if(activeStored.legnth !== stored.legnth) {
...
async function fetchWeather() {
  ...
  if(API_KEY === 'fe554f8a2204ce05c44ebc8abeabd693') {
    document.getElementById('weatherDesc').textContent = 'Heavy rainfall expected — sign up at openweathermap.org to go live';
    document.getElementById('weatherDesc').classList.remove('italic', 'text-muted');
  ...
  try{
    ...
    if(mainWeather === 'Rain' || mainWeather === 'Thunderstorm'){
      document.getElementById('weatherBanner').classList.replace('border-1-sky', 'border-1-amber');
    }
